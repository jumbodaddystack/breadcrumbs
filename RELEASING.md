...
1. Bump `version` in `pyproject.toml` if needed (currently `0.1.0`).
2. Tag and create a **GitHub Release** (e.g. `v0.1.0`). Publishing the release
   triggers the `publish-pypi` job automatically.

```bash
git tag v0.1.0
git push origin v0.1.0
# then publish the release in the GitHub UI (or `gh release create v0.1.0`)
```
